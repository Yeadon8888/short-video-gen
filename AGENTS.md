...
This file explains how an agent should work in this repository.

## Repository Identity

This repository is the standalone Next.js web application for VidClaw v2.

- This directory is the only project root that matters for the web app
- Do not assume there is an outer wrapper repo or a parent skill project
- Ignore any historical Python skill notes unless the user explicitly asks about them
- For this repo, the source of truth is `src/`, `scripts/`, `drizzle/`, `public/`, and the current `README.md`
...
- BLTCY / Plato-compatible video generation API
- TikHub for Douyin/TikTok source video resolution
- Upload gateway for user assets
...
1. Frontend sends `POST /api/generate` or `POST /api/generate/batch`
2. Server authenticates, checks credits, resolves inputs, calls Gemini, then creates provider tasks
3. Frontend receives streaming logs or batch creation response
4. Frontend polls generation status and task pages refresh stale items
5. Cron routes handle scheduled work and timeout cleanup
...
- `src/app/api/tasks/groups/[groupId]/downloads/route.ts`
...
- `src/lib/video/providers/plato.ts`
- `src/lib/tikhub.ts`
...
- This repo is meant to be deployed from this directory directly
- Local Vercel link metadata lives in `.vercel/project.json`
- `vercel.json` at the repo root is the active deployment config
- Preferred release path:
  - `npm run deploy:vercel:check`
  - `npm run release:vercel`
- `npm run deploy:vercel` now includes production database migration before deployment
- Do not rely on raw `vercel --prod` unless you have already verified that the remote project `Root Directory` is still the repo root
...
- If touching generation lifecycle logic, inspect:
  ...
  - `src/app/api/tasks/refresh/route.ts`
  - `src/app/api/cron/timeout/route.ts`
  - `src/lib/tasks/reconciliation.ts`
  - `src/lib/tasks/fulfillment.ts`
- Do not assume parent directories contain relevant runtime code
- Keep this repo self-contained and document any new entrypoints in `README.md`
...
- 仅在子代理 `BLOCKED`、需修改 `scope_write`、需调整共享 contract / shared types / schema、出现写冲突或依赖冲突、或确需用户验收 / 决策时才打断确认。
