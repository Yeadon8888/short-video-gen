 ...
 * watermark-free video. Mirrors the Python generate.py logic.
 */
...
/** Return true if the text looks like a Douyin / TikTok share link. */
...
/**
 * Resolve a Douyin/TikTok share URL, obtain watermark-free video URL via
 * TikHub APIs, and download the video into memory.
 */
export async function downloadVideoFromUrl(
  ...
  try {
    const result = await tikhubGet(`${HYBRID_PATH}?url=${encoded}&minimal=true`);
    const code =
      (result.code as number) ??
      ((result.detail as Record<string, unknown>)?.code as number);
    if (code === 200) {
      const vd = (result.data as Record<string, Record<string, unknown>>)?.video_data ?? {};
  ...
  if (!videoUrl) {
    ...
    if (m) {
      ...
      if ((result.code as number) === 200) {
        const detail =
          (result.data as Record<string, Record<string, unknown>>)?.aweme_detail ?? {};
  ...
  if (sizeMB < 0.1) {
    throw new Error(`下载文件过小 (${sizeMB.toFixed(2)} MB)，可能下载失败`);
  }
