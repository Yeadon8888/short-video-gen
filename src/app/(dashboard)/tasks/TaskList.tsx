...
function renderExpiryCountdown(createdAt: string | Date) {
  const days = getDaysUntilVideoExpiry(createdAt);
  return (
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2 rounded-lg border border-amber-500/20 bg-amber-500/5 px-4 py-2.5">
        ...
        <p className="text-sm text-amber-300/90">
          生成的视频将在 <span className="font-medium text-amber-200">{VIDEO_EXPIRY_DAYS} 天</span> 后自动清除，请及时下载保存
        </p>
      </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-sm ${cfg.color}`}>
                      {task.status === "done" && taskVideoSummary.plannedCount > 1
                        ? `${taskVideoSummary.successCount}/${taskVideoSummary.plannedCount} 完成`
                        : cfg.label}
                    </span>
                    <span className="rounded-[var(--vc-radius-sm)] bg-[var(--vc-bg-elevated)] px-1.5 py-0.5 text-xs text-[var(--vc-text-muted)]">
                      {getTaskSourceModeLabel((task.paramsJson as TaskParamsSnapshot | null)?.sourceMode, task.type)}
                    </span>
                    {task.creditsCost > 0 && (
                      <span className="text-xs text-[var(--vc-text-muted)]">
                        -{task.creditsCost} 积分
