...
| `--submit` | 仅提交任务不等待结果，状态保存到 `.task_state.json`（适用于 OpenClaw 等短会话环境） | 否 |
| `--check [task_ids]` | 快速检查任务状态（不阻塞）。不传参数则从上次 `--submit` 的状态恢复 | — |
...
**场景 8：OpenClaw 短会话模式（分步执行）**
```bash
cd "$HOME/.claude/skills/short-video-gen"
# 第一步：提交任务（~60秒内完成，立即返回 task_ids）
...
# 或指定 task_ids 检查
python3 scripts/generate.py --check "veo3.1-fast:xxx,veo3.1-fast:yyy"
```
...
- **OpenClaw 等短会话环境**：使用 `--submit` + `--check` 分步执行，避免长连接超时
- 轮询时显示任务进度百分比（如 `progress=67%`），方便 agent 判断状态
