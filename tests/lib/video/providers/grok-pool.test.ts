...
test("pickQueueDrainCandidates returns tasks in FIFO order under fairness cap", () => {
  ...
  const picked = pickQueueDrainCandidates(candidates, { budget: 10, fairnessCap: 10 });
...
test("pickQueueDrainCandidates demotes tasks beyond per-user cap", () => {
  ...
  const picked = pickQueueDrainCandidates(candidates, { budget: 13, fairnessCap: 10 });
...
test("pickQueueDrainCandidates respects budget", () => {
  ...
  assert.deepEqual(pickQueueDrainCandidates(candidates, { budget: 1, fairnessCap: 10 }), ["t1"]);
});
