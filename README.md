...
- 参考图上传与管理
- 可配置的视频模型与积分消耗
- 用户自定义 Prompt 与后台系统 Prompt 配置
- 任务列表、轮询刷新、失败退款
- 定时托管生成和超时清理
- 管理后台：模型、系统 Prompt、任务、积分、公告
...
- 后台系统 Prompt 配置：`src/app/api/admin/system-prompts/route.ts`
...
- 支持管理员在 `/admin/system-prompts` 统一查看和修改代码默认系统 Prompt
